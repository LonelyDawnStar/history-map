(() => {
  ...
  const KO_NAMES = {
    ...
    'Sao Paulo':'상파울루','Buenos Aires':'부에노스아이레스','Rio de Janeiro':'리우데자네이루','Brasilia':'브라질리아','Brasília':'브라질리아',
    'Caracas':'카라카스','Johannesburg':'요하네스버그','Cape Town':'케이프타운','Nairobi':'나이로비','Addis Ababa':'아디스아바바',
    'Khartoum':'하르툼','Alexandria':'알렉산드리아','Casablanca':'카사블랑카','Algiers':'알제','Sydney':'시드니',
    'Melbourne':'멜버른','Perth':'퍼스','Brisbane':'브리즈번','Auckland':'오클랜드','Taipei':'타이베이','Chengdu':'청두',
    'Chongqing':'충칭','Wuhan':'우한','Tianjin':'톈진','Nanjing':'난징','Hangzhou':'항저우','Xi’an':'시안','Xian':'시안',
    'Harbin':'하얼빈','Shenyang':'선양','Busan':'부산','Bangalore':'벵갈루루','Bengaluru':'벵갈루루','Chennai':'첸나이',
    'Hyderabad':'하이데라바드','Ahmedabad':'아마다바드','Pune':'푸네','Surat':'수라트','Yangon':'양곤','Hanoi':'하노이',
    'Kuala Lumpur':'쿠알라룸푸르','Phnom Penh':'프놈펜','Dubai':'두바이','Abu Dhabi':'아부다비','Jeddah':'제다',
    'Damascus':'다마스쿠스','Amman':'암만','Tel Aviv-Yafo':'텔아비브','Jerusalem':'예루살렘','Ankara':'앙카라','Tashkent':'타슈켄트',
  ...
  function visibleBorderHash() {
    ...
    for (const border of state.borders) {
      ...
      for (let i = 0; i < points.length; i += 4) {
        const p = points[i];
        hash ^= ((p?.[0] || 0) * 10) | 0; hash = Math.imul(hash, 16777619);
        hash ^= ((p?.[1] || 0) * 10) | 0; hash = Math.imul(hash, 16777619);
      }
      if (points.length) {
  ...
  function ownershipKey() {
    const countries = state.countries.map(c => c.id).join(',');
    const fills = state.fills.filter(yearVisible).map(f => `${f.id}:${f.countryId}:${f.x}:${f.y}`).join('|');
    return `${state.year};${countries};${fills};${visibleBorderHash()}`;
  }

  function currentOwnerMap() {
    if (!landMask) return null;
    const key = ownershipKey();
    if (key === ownerCacheKey && ownerCacheMap) return ownerCacheMap;
  ...
  function isMajorReferenceCity(city) {
    ...
    return !!city.name && (pop >= 4500000 || (city.isNationalCapital && pop >= 2500000 && city.scalerank <= 2));
  }

  function applyNameVisibility() {
    document.body.classList.toggle('hide-map-names', showNames?.checked === false);
  }

  function renderKey(tier) {
    const custom = (state.cities || []).filter(yearVisible).map(c => `${c.id}:${c.x}:${c.y}:${c.name}`).join('|');
    const capitals = state.countries.map(c => `${c.id}:${c.capitalCityId || ''}`).join('|');
    return [tier, showCities?.checked !== false ? 1 : 0, showRivers?.checked !== false ? 1 : 0, showMountains?.checked !== false ? 1 : 0, ownershipKey(), custom, capitals].join(';');
  }
  ...
  function renderCityGroups(cities, ownerMap) {
    ...
    groups.attr('transform', d => `translate(${d.x},${d.y})`).classed('custom-city', d => !!d.custom).style('pointer-events', d => d.custom ? 'all' : 'none');
  ...
  function renderGeographyNow(force = false) {
    applyNameVisibility();
    if (!ready || !projection || !geoPath) return;
    const tier = zoomTier();
    const key = renderKey(tier);
    ...
    if (showRivers?.checked !== false) {
      ...
      geographyLayer.selectAll('path.reference-river').data(rivers, d => d.properties?.ne_id || `${d.properties?.name}-${d.properties?.scalerank}`)
        .join('path').attr('class', 'reference-river').attr('d', geoPath);
      const labeled = tier === 0 ? [] : rivers.filter(f => koreanName(f.properties) && Number(f.properties?.scalerank ?? 99) <= Math.max(2, riverRankLimit(tier) - 2));
      geographyLayer.selectAll('text.reference-river-label').data(labeled, d => d.properties?.ne_id || `${d.properties?.name}-label`)
        .join('text').attr('class', 'reference-label reference-river-label map-name')
        .attr('x', d => geoPath.centroid(d)[0]).attr('y', d => geoPath.centroid(d)[1]).text(d => koreanName(d.properties));
    }
    ...
    if (showMountains?.checked !== false) {
      ...
      const groups = geographyLayer.selectAll('g.reference-mountain').data(mountains, d => d.properties?.ne_id || `${d.properties?.name}-${d.geometry?.coordinates?.join(',')}`)
        .join(enter => {
          const g = enter.append('g').attr('class', 'reference-mountain');
          g.append('text').attr('class', 'mountain-symbol').text('▲');
          g.append('text').attr('class', 'reference-label reference-mountain-label map-name').attr('x', 7).attr('y', 4);
          return g;
        });
      ...
      groups.select('.reference-mountain-label').style('display', tier === 0 ? 'none' : null).text(d => koreanName(d.properties));
    }
  ...
  function scheduleRender(force = false) {
    ...
    renderTimer = setTimeout(() => renderGeographyNow(force), 90);
  }

  showNames?.addEventListener('change', applyNameVisibility);
  [showCities, showRivers, showMountains].forEach(input => input?.addEventListener('change', () => renderGeographyNow(true)));
  ...
  async function loadReferenceGeography() {
    try {
      const [worldRes, cityRes, riverRes, regionRes] = await Promise.all([fetch(DATA.land), fetch(DATA.cities), fetch(DATA.rivers), fetch(DATA.regions)]);
      if (![worldRes, cityRes, riverRes, regionRes].every(r => r.ok)) throw new Error('reference geography load failed');
      const [world, cityData, riverData, regionData] = await Promise.all([worldRes.json(), cityRes.json(), riverRes.json(), regionRes.json()]);
      const land = topojson.feature(world, world.objects.land);
      projection = d3.geoNaturalEarth1().fitExtent([[22,24],[1178,596]], land);
      geoPath = d3.geoPath(projection);

      const referenceCities = (cityData.features || []).map((feature, index) => {
      ...
      mountainFeatures = (regionData.features || []).filter(feature => {
        ...
        return koreanName(feature.properties) && /mountain|range|plateau|highland|hill|massif/i.test(`${cls} ${raw}`);
      });

      ready = true;
      lastZoomTier = zoomTier();
      invalidateOwnership();
      renderGeographyNow(true);
