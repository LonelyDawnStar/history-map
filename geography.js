(() => {
  ...
  toolHelp.city = '도시 도구: 육지를 누르면 주요 도시를 추가합니다. 현재 영토 기준으로 소속 국가가 자동 판정됩니다.';
  ...
  function countryAtFromOwner(x, y, owner) {
  ...
  function countryAt(x, y) {
    return countryAtFromOwner(x, y, currentOwnerMap());
  }

  function citiesForCountry(countryId) {
    const owner = currentOwnerMap();
    return state.cities.filter(city =>
      yearVisible(city) && countryAtFromOwner(city.x, city.y, owner)?.id === countryId
    );
  }
  ...
  function renderGeography() {
    ...
    const rivers = state.rivers.filter(yearVisible);
    ...
    geographyLayer.selectAll('text.river-label')
      .data(rivers.filter(d => d.name), d => d.id)
      .join('text')
    ...
    const mountains = state.mountains.filter(yearVisible);
    ...
    geographyLayer.selectAll('text.mountain-label')
      .data(mountains.filter(d => d.name), d => d.id)
      .join('text')
    ...
    groups.each(function(city) {
      const owner = countryAtFromOwner(city.x, city.y, ownerMap);
  ...
  function drawFeatureDraft() {
    ...
    draftLayer.append('path')
      .attr('class', `geo-draft geo-draft-${drawingFeature}`)
      .attr('d', line(featurePoints));
  }
  ...
  svg.on('pointercancel.geography', cancelFeature);
