(() => {
  ...
  function findTargetBorder(){
    ...
    const start=stroke[0],end=stroke.at(-1),radius=snapRadius(); let best=null;
  ...
  function showStatus(message){
    statusEl.style.opacity='1'; statusEl.textContent=message;
    setTimeout(()=>statusEl.style.opacity='0',1900);
  }

  function commitAdjustment(){
    const target=findTargetBorder();
    if(!target){showStatus('영토 안에서 시작해 같은 기존 국경에 다시 닿게 그려 주세요.');return false;}
    if(!ownerBeforeStroke||!startCountryId)return false;

    const points=target.border.points;
    const low=Math.min(target.startIndex,target.endIndex),high=Math.max(target.startIndex,target.endIndex);
    const replacement=target.startIndex<=target.endIndex?stroke:[...stroke].reverse();
    const merged=[...points.slice(0,low+1),...replacement.slice(1,-1).map(p=>[+p[0].toFixed(2),+p[1].toFixed(2)]),...points.slice(high)];
    ...
    renderBorders(); renderTerritories();
  ...
  button.addEventListener('click',()=>{
    helpEl.textContent='국경 조정: 늘릴 국가의 영토 안에서 시작해 기존 국경을 넘어 원하는 경로를 그린 뒤 같은 국경에 다시 닿게 하세요.';
  ...
  svg.on('pointerdown.borderAdjust',event=>{
    ...
    // Use the renderer cache directly. No flood-fill/canvas work is needed on pointerdown.
    ...
    // Strict rule: the first pointer pixel itself must belong to a country.
    // No nearby-country guessing, so starting on a border or unowned land cannot begin adjustment.
    if(ci<0){
    ...
    drawing=true; pointerId=event.pointerId; stroke=[];
  ...
  svg.on('pointermove.borderAdjust',event=>{
    ...
    event.preventDefault(); addPoint(mapPoint(event)); drawPreview();
  });
  ...
  function finish(event){
    ...
    stroke=[]; startCountryId=null; ownerBeforeStroke=null;
    draftLayer.selectAll('.border-adjust-preview').remove();
  }
